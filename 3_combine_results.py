...
combined = False
if combined:
    with open(output_dir + output_folder + "/results/results_"+str(output_var)+'_'+str(image_radius)+"_"+str(linear_coef)+
              "/model_output_hyper_searching_dic.pkl", "rb") as f:
        model_output_hyper_searching = pickle.load(f)
else:  
    model_output_hyper_searching = {}
    files = glob.glob(output_dir+output_folder+'/results/results_'+str(output_var)+'_'+str(image_radius)+"_"+str(linear_coef)+
                  '/model_output_hyper_searching_dic_*.pickle')
...
cnn_performance_table = pd.DataFrame(np.array([[i for i in range(n_hyperparam_searching)], cnn_train_loss_list, \
    ...
    ascending=True).to_csv(output_dir+output_folder+'/results/cnn_performance_table_'+str(output_var)+'_'+str(image_radius)+'.csv', index=False)

if not combined:
    with open(output_dir + output_folder + "/results/results_"+str(output_var)+'_'+str(image_radius)+"_"+str(linear_coef)+
              "/model_output_hyper_searching_dic_"+str(output_var)+'_'+str(image_radius)+".pkl", "wb") as f:
...
hyperparam_table = pd.DataFrame(df, columns = hyperparam_names+('val','output_date')).to_csv(output_dir + output_folder +
                    "/results/results_"+str(output_var)+'_'+str(image_radius)+"_"+str(linear_coef)+
